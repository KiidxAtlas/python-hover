...
export class InventoryFetcher {
  ...
  async hydrateCachedInventories(): Promise<string[]> {
    ...
    await Promise.all(
      cachedPackages
        ...
        .map(packageName =>
          this.ensurePackageLoaded(packageName, undefined, packageName === 'builtins').catch(
            () => undefined,
          ),
        ),
    )
      ...
      if (key.name.includes('.')) {
        ...
        for (let i = nameParts.length - 2; i >= 1; i--) {
          ...
          if (packageInventory.has(candidate)) {
            getEngineLogger().log(`Inventory: Strategy 5 matched ${key.name} → ${candidate}`)
      ...
      if (key.qualname && !key.qualname.includes('.') && key.module) {
        ...
        if (moduleDepth >= 2) {
          ...
          if (candidates.length === 1) {
            getEngineLogger().log(`Inventory: Strategy 6 matched ${key.name} → ${candidates[0].name}`)
  ...
  private async loadInventory(
    ...
    if (cached) {
      try {
        ...
        getEngineLogger().log(`Loaded inventory for ${packageName} from disk cache`)
        return
      } catch (e) {
        getEngineLogger().log(`Failed to parse cached inventory for ${packageName}`)
      }
    }
    ...
    if (!this.allowNetwork) {
      getEngineLogger().log(`Network disabled, skipping inventory fetch for ${packageName}`)
      const empty = new Map<string, HoverDoc>()
      this.cache.set(inventoryCacheName, empty)
      if (packageName !== inventoryCacheName) {
        this.cache.set(packageName, empty)
    ...
    try {
      ...
      this.packageBaseUrls.set(packageName, baseUrl)
      getEngineLogger().log(`Loaded inventory for ${packageName}: ${inventory.size} items`)
      ...
      if (!this.failedInventoryLog.has(packageName)) {
        ...
        getEngineLogger().log(`Failed to load inventory for ${packageName} from ${inventoryUrl}: ${e}`)
      }
      ...
      try {
        ...
        isPythonDocsCanonical =
          parsed.hostname === 'docs.python.org' && parsed.pathname.startsWith('/3/')
      } catch {
        /* non-URL format */
      }
      if (isStdlib && !isPythonDocsCanonical) {
        ...
        try {
          getEngineLogger().log(`Retrying stdlib inventory with canonical URL: ${fallbackUrl}`)
          const buffer = await this.fetchBuffer(fallbackUrl)
          const docsProvider = await this.resolveDocsProvider(fallbackBase).catch(
            (): 'sphinx' => 'sphinx',
          )
          const inventory = this.sanitizeInventory(
          ...
          getEngineLogger().log(`Loaded stdlib inventory from canonical URL: ${inventory.size} items`)
          const obj = Object.fromEntries(inventory)
          this.diskCache.set(cacheKey, JSON.stringify(obj))
          return
        } catch (fallbackErr) {
          getEngineLogger().log(`Canonical stdlib inventory also failed: ${fallbackErr}`)
        }
      }
  ...
  getModuleExports(packageName: string, limit = 16): string[] {
    ...
    return entries
      .sort((a, b) => (a.depth !== b.depth ? a.depth - b.depth : a.kindScore - b.kindScore))
      .slice(0, limit)
      .map(e => e.label)
  }
    ...
    for (const [pkg, inventory] of candidateInventories) {
      for (const [name, doc] of inventory) {
        ...
        if (
          !best ||
          score > best.score ||
          (score === best.score && summary.name.length < best.summary.name.length)
        ) {
          best = { score, summary }
        }
      }
  ...
  getModuleSymbols(moduleName: string, maxSymbols = 5000): IndexedSymbolSummary[] {
    ...
    const finalResults = results.sort((a, b) => a.name.localeCompare(b.name)).slice(0, maxSymbols)
  ...
  private scoreReferenceCandidate(candidate: {
    ...
    const { label, url, currentModule, currentOwner, preferredPackage, packageName, name, doc } =
      candidate
    ...
    if (
      currentOwner &&
      normalizedName.endsWith(`.${currentOwner.toLowerCase()}.${normalizedLabel}`)
    ) {
