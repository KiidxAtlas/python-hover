...
const INDEX_REQUEST_HEADERS = {
  "User-Agent":
    "Mozilla/5.0 (compatible; PyHover/0.7; +https://github.com/KiidxAtlas/python-hover)",
...
export class SiteIndexResolver {
  ...
  private async loadEntriesUncached(
    baseUrl: string,
  ): Promise<SiteIndexEntry[] | null> {
  ...
  private parseSphinxIndex(text: string, baseUrl: string): SiteIndexEntry[] {
    ...
    for (const [prefix, value] of Object.entries(objects)) {
      ...
      for (const [name, rawMeta] of Object.entries(
        ...
        const docIndex =
          typeof rawMeta[0] === "number" ? rawMeta[0] : Number(rawMeta[0]);
        if (
          !Number.isInteger(docIndex) ||
          docIndex < 0 ||
          docIndex >= filenames.length
        ) {
          continue;
        }
        ...
        const anchor =
          typeof rawMeta[3] === "string" && rawMeta[3] && rawMeta[3] !== "-"
            ? `#${rawMeta[3]}`
            : "";
        ...
        entries.push({
          ...
          url: new URL(
            `${filename}${anchor}`,
            this.withTrailingSlash(baseUrl),
          ).toString(),
          summary: undefined,
  ...
  private inferSphinxKind(rawType: unknown): string | undefined {
    ...
    if (normalized.includes("attribute") || normalized.includes("property")) {
      return "attribute";
    }
    return undefined;
  }
  ...
  private scoreEntry(entry: SiteIndexEntry, key: DocKey): number {
    ...
    if (symbolName === fullName || symbolName === qualname) {
      score = Math.max(score, 1000);
    }
    if (title === fullName || title === qualname) {
      score = Math.max(score, 960);
    }
    if (symbolName === `${moduleName}.${leaf}`) {
      score = Math.max(score, 920);
    }
    if (title === `${moduleName}.${leaf}`) {
      score = Math.max(score, 880);
    }
    if (symbolName === leaf || title === leaf) {
      score = Math.max(score, 820);
    }
    if (
      leaf &&
      (symbolName?.endsWith(`.${leaf}`) || title.endsWith(`.${leaf}`))
    ) {
      score = Math.max(score, 720);
    }
    if (
      qualname === packageName &&
      (title === packageName || symbolName === packageName)
    ) {
      score = Math.max(score, 900);
    }
    if (title.includes(qualname)) {
      score = Math.max(score, 680);
    }
    if (summary.includes(qualname)) {
      score = Math.max(score, 580);
    }
    if (title.includes(leaf)) {
      score = Math.max(score, 520);
    }
    if (summary.includes(leaf)) {
      score = Math.max(score, 420);
    }
    if (
      moduleName &&
      title.includes(moduleName) &&
      (title.includes(leaf) || summary.includes(leaf))
    ) {
      score += 40;
    }
  ...
  private inferKindFromTitle(title: string): string | undefined {
    ...
    if (/\bmodule\b/i.test(normalized)) {
      return "module";
    }
    if (/^[A-Z][A-Za-z0-9_]+(?:\.[A-Z][A-Za-z0-9_]+)*$/.test(normalized)) {
      return "class";
    }
    if (/\(\)$/.test(normalized) || /\bfunction\b/i.test(normalized)) {
      return "function";
    }
    return undefined;
  }
