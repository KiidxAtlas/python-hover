...
function isLikelyRepositoryUrl(value: string | null | undefined): boolean {
  ...
  try {
    ...
    if (
      !/github\.com$|gitlab\.com$|bitbucket\.org$|codeberg\.org$|sourcehut\.org$/.test(
        host,
      )
    ) {
      return false;
...
function scoreRepoCandidate(label: string, url: string): number {
  ...
  if (
    lowerLabel === "source" ||
    lowerLabel === "source code" ||
    lowerLabel === "repository" ||
    lowerLabel === "code"
  ) {
    score += 100;
  }
  if (
    ...
    lowerLabel.includes("gitlab") ||
    lowerLabel.includes("bitbucket")
  ) {
    score += 70;
  }
  if (
    ...
    lowerLabel === "home" ||
    lowerLabel === "home page" ||
    lowerLabel === "website"
  ) {
    score += 15;
  }
  if (
    lowerLabel.includes("bug") ||
    lowerLabel.includes("issue") ||
    lowerLabel.includes("changelog") ||
    lowerLabel.includes("release")
  ) {
    score -= 40;
  }
...
function pickBestProjectUrl(
  ...
  const labelScore = (label: string): number => {
    ...
    if (l.includes("documentation") || l.includes("doc ") || l === "doc") {
      return 1;
    }
    if (l.includes("readthedocs") || l.includes("rtd")) {
      return 2;
    }
    if (
      l.includes("homepage") ||
    ...
    if (l.includes("changelog") || l.includes("release")) {
      return 6;
    }
    if (
  ...
  if (entries.length > 0) {
    entries.sort(
      (a, b) => labelScore(a[0]) - labelScore(b[0]) || a[0].localeCompare(b[0]),
    );
    return entries[0][1];
  }
...
export class PyPiClient {
  ...
  async getPackageMetadata(packageName: string): Promise<{
    url: string | null;
    summary: string | null;
    links: Record<string, string>;
    version: string | null;
    ...
    try {
      const metadata = await this.fetchJson<PyPiPackageResponse>(
        `https://pypi.org/pypi/${packageName}/json`,
      );
      if (!metadata || !metadata.info) {
        if (this.diskCache) {
          this.diskCache.setCorpusPackageMetadata(packageName, {
            negative: true,
          });
        }
        return EMPTY;
      ...
      const bestUrl = pickBestProjectUrl(
        links,
        typeof info.home_page === "string" ? info.home_page : null,
      );
      ...
      const license: string | null =
        rawLicense && rawLicense !== "UNKNOWN" && rawLicense !== "NOASSERTION"
          ? rawLicense
          : null;
      const requiresPython: string | null =
        typeof info.requires_python === "string" && info.requires_python.trim()
          ? info.requires_python.trim()
          : null;
  ...
  async findDocs(key: DocKey): Promise<HoverDoc | null> {
    const { url, summary, links, version, license, requiresPython } =
      await this.getPackageMetadata(key.package);
    if (url || summary) {
      return {
        title: key.package,
