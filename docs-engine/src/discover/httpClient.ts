...
/** Validate that a URL is safe to fetch (no SSRF via localhost/file/etc). */
function isSafeUrl(url: string): boolean {
  try {
    ...
    // Block localhost / private IP redirects (SSRF protection).
    ...
    const ip = hostname.replace(/^[\[\]]/g, "").replace(/[\]]/g, "");
    if (/^\d+\.\d+\.\d+\.\d+$/.test(ip)) {
      ...
      const req = https.get(
        ...
        (res) => {
          if (
            ...
            // SSRF protection: reject redirects to localhost/private IPs.
            if (!isSafeUrl(redirectUrl)) {
              reject(new Error("Redirect blocked: SSRF protection"));
              return;
            }
            attemptRequest(redirectUrl, redirectCount + 1, attempt)
              .then(resolve)
              .catch(reject);
            return;
          }
...
export function httpHeadExists(
  url: string,
  options?: HttpClientOptions,
): Promise<boolean> {
  const opts = resolveOptions(options);
