...
export class SiteIndexResolver {
  ...
  private parseSphinxIndex(text: string, baseUrl: string): SiteIndexEntry[] {
    ...
    for (const [prefix, value] of Object.entries(objects)) {
      ...
      for (const [name, rawMeta] of Object.entries(value as Record<string, unknown>)) {
        ...
        if (!Number.isInteger(docIndex) || docIndex < 0 || docIndex >= filenames.length) {
          continue
        }
        ...
        const anchor =
          typeof rawMeta[3] === 'string' && rawMeta[3] && rawMeta[3] !== '-' ? `#${rawMeta[3]}` : ''
  ...
  private scoreEntry(entry: SiteIndexEntry, key: DocKey): number {
    ...
    if (leaf && (symbolName?.endsWith(`.${leaf}`) || title.endsWith(`.${leaf}`))) {
      score = Math.max(score, 720)
    }
    if (qualname === packageName && (title === packageName || symbolName === packageName)) {
