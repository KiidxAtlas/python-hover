...
export class SphinxScraper {
  ...
  getCachedContent(packageName: string, url: string, group?: string): string | null {
    return (
      this.diskCache.getCorpusEntry(packageName, this.normalizeCorpusUrl(url), group)?.content ??
      null
    )
  }
  ...
  getCachedStructuredContent(
    ...
    return (
      this.diskCache.getCorpusEntry(packageName, this.normalizeCorpusUrl(url), group)
  ...
  getCachedSeeAlso(packageName: string, url: string, group?: string): string[] | null {
    return (
      this.diskCache.getCorpusEntry(packageName, this.normalizeCorpusUrl(url), group)?.seeAlso ??
      null
    )
  }
  ...
  deriveStructuredContent(markdown: string): StructuredHoverContent | undefined {
    ...
    for (const [blockIndex, block] of blocks.entries()) {
      ...
      if (listItems) {
        const sectionRole =
          role === 'note' ? 'note' : role === 'example' ? 'example' : 'description'
        sections.push({
          kind: 'list',
          role: sectionRole,
          title: titleForSection,
  ...
  private extractPageSummary(html: string, pageUrl: string): string | null {
    ...
    const sectionMatch = /<section[^>]*id=["']module-[^"']*["'][^>]*>([\s\S]*?)<\/section>/i.exec(
      html,
    )
    if (sectionMatch) {
  ...
  deriveSeeAlsoFromMarkdown(markdown: string): string[] {
    ...
    const items = blocks.map(block => this.extractSeeAlsoItems(block) ?? []).flat()
  ...
  private isSignatureLikeParagraph(text: string): boolean {
    ...
    return (
      /^(?:class|async\s+def|def)\s+[A-Za-z_][\w.]*\s*\(.*\)\s*(?:->\s*.+)?$/i.test(normalized) ||
  ...
  private normalizeExtractedMarkdown(markdown: string): string {
    ...
    cleaned = cleaned.replace(/^`{1,3}\s*[A-Za-z_][\w.]*\s*`{0,3}\s*$/m, match =>
      match.trim().length <= 32 ? '' : match,
    )
  ...
  private htmlToMarkdown(html: string, pageUrl: string): string {
    ...
    md = md.replace(/<ol[^>]*>([\s\S]*?)<\/ol>/gim, (_, inner) => {
      let n = 0
      return (
        '\n' + inner.replace(/<li[^>]*>/gim, () => `${++n}. `).replace(/<\/li>/gim, '\n') + '\n'
      )
    })
