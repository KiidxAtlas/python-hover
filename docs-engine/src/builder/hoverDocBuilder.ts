...
import { DocstringParser, ParsedDocstring } from "../process/docstringParser";
...
export class HoverDocBuilder {
  ...
  build(symbolInfo: SymbolInfo, docs: HoverDoc | null): HoverDoc {
    ...
    const signatureHint = symbolInfo.signature || structuredContent?.signature;
    const structuredParameters =
      this.extractParametersFromStructuredContent(structuredContent);
    const structuredReturns =
      this.extractReturnFromStructuredContent(structuredContent);
    const structuredRaises =
      this.extractRaisesFromStructuredContent(structuredContent);

    const title = this.buildTitle(symbolInfo, signatureHint);
    const signature =
      symbolInfo.kind === "keyword"
        ? undefined
        : this.sanitizeSignature(
            symbolInfo.signature ||
              structuredContent?.signature ||
    ...
    if (
      legacyContent &&
      PLACEHOLDER_MSGS.some(
        (p) => legacyContent!.startsWith(p) || legacyContent === p,
      )
    ) {
  ...
  private buildTitle(symbolInfo: SymbolInfo, signatureHint?: string): string {
    ...
    const callableFromSignature =
      this.extractCallableFromSignature(signatureHint);

    if (name && symbolInfo.kind === "method" && !name.includes(".")) {
      const callableLeaf = callableFromSignature?.split(".").pop();
      if (callableLeaf && callableLeaf !== name) {
        return `${name}.${callableLeaf}`;
      }
    }
    ...
    if (
      name &&
      !name.includes(".") &&
      callableFromSignature?.startsWith(`${name}.`)
    ) {
      return callableFromSignature;
    }
  ...
  private extractCallableFromSignature(signature?: string): string | undefined {
    const normalized = signature?.trim();
    if (!normalized) {
      return undefined;
    }

    const withoutKind = normalized.replace(/^\([a-z][a-z0-9_]*\)\s+/i, "");
    const defMatch = /^(?:async\s+def|def)\s+([A-Za-z_][\w.]*)\s*\(/.exec(
      withoutKind,
    );
    if (defMatch?.[1]) {
      return defMatch[1];
    ...
    const genericMatch = /^([A-Za-z_][\w.]*)\s*[[(]/.exec(withoutKind);
    if (genericMatch?.[1]) {
  ...
  private buildSummary(
    ...
    if (
      content &&
      PLACEHOLDER_MSGS.some((p) => content!.startsWith(p) || content === p)
    ) {
      content = undefined;
    }
    ...
    if (symbolInfo.kind !== "keyword") {
      ...
      if (
        raw.length >= 48 &&
        !this.isUnhelpfulRuntimeFallback(symbolInfo.docstring!)
      ) {
        if (
          parsedSummary &&
          !this.shouldPreferRawDocstring(parsedSummary, symbolInfo.docstring)
        ) {
          return parsedSummary;
        }
        if (
          docstringSummary &&
          (this.isGoodSummary(docstringSummary) || !summaryFromDocs)
        ) {
    ...
    if (
      contentSummary &&
      (this.isGoodSummary(contentSummary) || !docstringSummary)
    ) {
    ...
    if (
      docstringSummary &&
      (this.isGoodSummary(docstringSummary) || !parsedSummary)
    ) {
      return docstringSummary;
    }
  ...
  private buildExamples(
    ...
    if (docs?.structuredContent?.examples) {
      docs.structuredContent.examples.forEach((example) =>
    ...
    if (parsed.examples) {
      parsed.examples.forEach((example: string) => examples.add(example));
    }
  ...
  private buildNotes(
    ...
    if (parsed.notes) {
      parsed.notes.forEach((note: string) => notes.add(note));
    }
  ...
  private buildBadges(symbolInfo: SymbolInfo): Badge[] {
    ...
    if (
      /[-:]\s*(Generator|Iterator|Iterable|AsyncGenerator|AsyncIterator)\b/.test(
        sig,
      )
    ) {
  ...
  private isDeprecatedDocstring(docstring: string): boolean {
    ...
    for (const rawLine of docstring.split("\n")) {
      ...
      if (
        /\bdeprecated since\b/.test(line) ||
        /\bis deprecated\b/.test(line) ||
        /\bhas been deprecated\b/.test(line)
      ) {
        return true;
      }

      if (
        /^deprecated(?:\s+since(?:\s+version)?\s+\S+)?\s*:\s*(?:use|replaced|removed|avoid|this|the|please|prefer|import|call|passing|parameter|method|function|class|module|path)\b/.test(
          line,
        )
      ) {
        return true;
  ...
  private shouldPreferRawDocstring(
    summary: string,
    rawDocstring?: string,
  ): boolean {
    if (!rawDocstring?.trim()) {
      return false;
    }
  ...
  private isGoodSummary(summary: string): boolean {
    ...
    if (
      /^(?:documentation|module|package|class|function)\b$/i.test(normalized)
    ) {
      return false;
    }
    if (this.isUnhelpfulRuntimeFallback(normalized)) {
      return false;
    }
    return true;
  }
  ...
  private isUnhelpfulRuntimeFallback(text: string): boolean {
    ...
    return (
      /^No Python documentation found for\s+/i.test(normalized) ||
      /^Use help\(\) to get the interactive help utility\.?/i.test(
        normalized,
      ) ||
  ...
  private extractPydocSummary(text: string): string | undefined {
    ...
    if (!summary || summary.startsWith("Methods defined")) {
      return undefined;
    }
  ...
  private normalizeSummaryLine(line: string): string | undefined {
    ...
    if (/^`{1,3}\s*[A-Za-z_][\w.]*\s*`{0,3}$/.test(trimmed)) {
      return undefined;
    }
    if (/^``\s+[A-Za-z_][\w.]*$/.test(trimmed)) {
      return undefined;
    }
    if (this.isUnhelpfulRuntimeFallback(trimmed)) {
      return undefined;
    }
    if (
      /^:(?:param|type|returns?|raises?|rtype|yield|ytype|example|examples)\b/i.test(
        trimmed,
      )
    ) {
      return undefined;
  ...
  private processInlineLinks(text: string): string {
    return text
      ...
      .replace(/:ref:`([^`\n]+)`/g, "$1")
      .replace(
        /:(?:func|class|meth|mod|attr|exc|data|const|type|obj):`([^`\n]+)`/g,
        "`$1`",
      );
  }
  ...
  private isSummarySectionBoundary(line: string, nextLine?: string): boolean {
    if (
      /^(?:Parameters|Returns|Raises|Examples?|Notes?|See Also|Args|Arguments|Attributes|Methods|Yields|Warns|References?)\s*:?$/i.test(
        line,
      )
    ) {
      return true;
  ...
  private extractSignatureFromContent(
    content?: string,
    title?: string,
  ): string | undefined {
    if (!content?.trim()) {
  ...
  private inferKind(symbolInfo: SymbolInfo, docs?: HoverDoc | null): string {
    ...
    if (symbolInfo.name === symbolInfo.module) {
      return "package";
    }

    if (symbolInfo.signature) {
      if (symbolInfo.signature.startsWith("class")) {
        return "class";
      }
      if (symbolInfo.signature.includes("def ")) {
        return "function";
      }
    }
  ...
  private extractParametersFromStructuredContent(
    structuredContent?: HoverDoc["structuredContent"],
  ) {
    if (!structuredContent?.sections?.length) {
      return undefined;
    }
  ...
  private extractReturnFromStructuredContent(
    structuredContent?: HoverDoc["structuredContent"],
  ) {
    if (!structuredContent?.sections?.length) {
      return undefined;
  ...
  private extractRaisesFromStructuredContent(
    structuredContent?: HoverDoc["structuredContent"],
  ) {
    if (!structuredContent?.sections?.length) {
      return undefined;
  ...
  private getStructuredFieldKind(
    title?: string,
  ...
  private parseStructuredParameter(
    content: string,
  ...
  private parseStructuredTypedEntry(
    content: string,
  ): { label: string; type?: string; description?: string } | undefined {
  ...
  private parseStructuredHeading(
    heading: string,
