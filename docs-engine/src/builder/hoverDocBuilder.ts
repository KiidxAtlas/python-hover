...
export class HoverDocBuilder {
  ...
  build(symbolInfo: SymbolInfo, docs: HoverDoc | null): HoverDoc {
    ...
    if (
      legacyContent &&
      PLACEHOLDER_MSGS.some(p => legacyContent!.startsWith(p) || legacyContent === p)
    ) {
  ...
  private isGoodSummary(summary: string): boolean {
    ...
    if (/^(?:documentation|module|package|class|function)\b$/i.test(normalized)) {
      return false
    }
  ...
  private normalizeSummaryLine(line: string): string | undefined {
    ...
    if (/^:(?:param|type|returns?|raises?|rtype|yield|ytype|example|examples)\b/i.test(trimmed)) {
      return undefined
    }
  ...
  private isMarkdownSectionBoundary(line: string): boolean {
    return (
      /^#{1,6}\s+/.test(line) || /^```/.test(line) || /^(?:Examples?|See Also|Notes?)\b/i.test(line)
    )
  }
  ...
  private parseStructuredHeading(heading: string): { label: string; type?: string } | undefined {
    ...
    const stripped = inner.replace(/^[-•]\s*/, '').replace(/\[([^\]]*)\]\(https?:[^)]*\)/g, '$1')
