import {
  stripDangerousTagBlocks,
  stripHtmlTags,
} from "../../../shared/htmlSanitizer";
import {
...
export class SphinxScraper {
  ...
  private shouldSkipUrl(url: string): boolean {
    try {
      ...
      return NON_CONTENT_PAGE_PATTERNS.some((pattern) =>
  ...
  private logSkippedFetch(url: string, reason: string): void {
    ...
    getEngineLogger().debug(`SphinxScraper skipped ${url}: ${reason}`);
  }
  ...
  getCachedContent(
    ...
    return (
      this.diskCache.getCorpusEntry(
        packageName,
        this.normalizeCorpusUrl(url),
        group,
      )?.content ?? null
    );
  }

  getCachedStructuredContent(
    packageName: string,
    url: string,
    group?: string,
  ): StructuredHoverContent | null {
    return (
      this.diskCache.getCorpusEntry(
        packageName,
        this.normalizeCorpusUrl(url),
  ...
  async fetchSeeAlso(
    ...
    try {
      ...
      const cached = forceRefresh
        ? null
        : this.getCachedSeeAlso(packageName, normalizedUrl, group);
      if (cached) {
        return cached;
      }
  ...
  async fetchContent(
    ...
    try {
      ...
      const cached = forceRefresh
        ? null
        : this.getCachedContent(packageName, normalizedUrl, group);
      if (cached) {
        return cached;
      }
      ...
      getEngineLogger().error(`SphinxScraper failed for ${url}:`, e);
      return null;
    }
  }

  deriveStructuredContent(
    markdown: string,
    ...
    for (const [blockIndex, block] of blocks.entries()) {
      ...
      const role = titleForSection
        ? this.inferSectionRole(titleForSection)
      ...
      const summaryCandidate = this.stripExampleLeadIn(
        normalized,
        blocks[blockIndex + 1],
      );
  ...
  private inferSectionRole(title?: string): StructuredHoverSection["role"] {
    ...
    if (/^(?:Examples?|Usage|Demo)\b/i.test(title)) {
      return "example";
    }
    if (
      /^(?:Note|Notes|Warning|Warnings|Caution|Important|Tip|Tips|Changed in version|New in version|Deprecated)\b/i.test(
        title,
      )
    ) {
      return "note";
  ...
  private extractPageSummary(html: string, pageUrl: string): string | null {
    ...
    const sectionMatch =
      /<section[^>]*id=["']module-[^"']*["'][^>]*>([\s\S]*?)<\/section>/i.exec(
        html,
      );
    if (sectionMatch) {
  ...
  private stripExampleLeadIn(text: string, nextBlock?: string): string {
    ...
    if (!/(?:For example|Examples?)\s*:\s*$/i.test(text)) {
      return text;
    }
    if (
      !this.isDoctestTranscriptBlock(nextBlock) &&
      !this.parseFencedCodeBlock(nextBlock)
    ) {
      return text;
    }
  ...
  private isNoteLikeParagraph(text: string): boolean {
    return (
      /^(?:Note|Warning|Caution|Important|Tip):\s+/i.test(text) ||
  ...
  private extractSection(
    ...
    if (tagName === "dt") {
      ...
      content =
        endSection !== -1
          ? rest.substring(0, endSection)
          : rest.substring(0, 5000);
    } else {
      const rest = html.substring(tagStart);
      ...
      content =
        nextHeader !== -1
          ? body.substring(0, nextHeader)
          : body.substring(0, 2000);
    }

    const md = this.normalizeExtractedMarkdown(
      this.htmlToMarkdown(content, pageUrl),
    );
    // Trim to a reasonable hover length
    if (md.length > 1500) {
      const cut = md.lastIndexOf("\n\n", 1500);
      return (
        (cut > 800 ? md.substring(0, cut) : md.substring(0, 1500)) + "\n\n…"
      );
    }
    return md || null;
  }
  ...
  private normalizeExtractedMarkdown(markdown: string): string {
    ...
    cleaned = cleaned.replace(
      /^`{1,3}\s*[A-Za-z_][\w.]*\s*`{0,3}\s*\n\n(?=```)/,
      "",
    );
    cleaned = cleaned.replace(/^``\s+[A-Za-z_][\w.]*\s*\n\n(?=```)/, "");
    cleaned = cleaned.replace(
      /^`{1,3}\s*[A-Za-z_][\w.]*\s*`{0,3}\s*$/m,
      (match) => (match.trim().length <= 32 ? "" : match),
    );
  ...
  private looksLikeFenceContent(line: string): boolean {
    ...
    if (this.isSignatureLikeParagraph(line)) {
      return true;
    }
    if (this.isGrammarLikeParagraph(line)) {
      return true;
    }
    if (line.includes("::=")) {
      return true;
    ...
    if (
      /^(?:>>>|\.\.\.|class\s|def\s|async\s+def\s|for\s|if\s|elif\s|while\s|with\s|try:|except\b|finally:|return\s|raise\s|yield\s|import\s|from\s)/.test(
        line,
      )
    ) {
      return true;
    }
    if (/^[A-Za-z_]\w*(?:\.\w+)*\s*=\s*[^=]/.test(line)) {
      return true;
    }
    return false;
  }
  ...
  private findBalancedClose(
    ...
    while (i < html.length && depth > 0) {
      ...
      if (sub.startsWith(closeTag)) {
        ...
        if (depth === 0) {
          return i;
        } // return start of closing tag so caller can exclude it
        i += closeTag.length;
        continue;
      }
  ...
  private htmlToMarkdown(html: string, pageUrl: string): string {
    ...
    md = md.replace(/<ul[^>]*>([\s\S]*?)<\/ul>/gim, (_, inner) => {
      return (
        "\n" +
        inner.replace(/<li[^>]*>/gim, "- ").replace(/<\/li>/gim, "\n") +
        "\n"
      );
    });
    ...
    md = md.replace(
      /<\/?(?:div|span|section|article|aside|nav|header|footer|main)[^>]*>/gim,
      "",
    );
