...
function canonicalizeRepositoryUrl(value: string | null | undefined): string | null {
  ...
  try {
    ...
    const rootSegments = segments[0].startsWith('@') ? segments.slice(0, 3) : segments.slice(0, 2)
...
function pickBestProjectUrl(links: Record<string, string>, homePage: string | null): string | null {
  ...
  const labelScore = (label: string): number => {
    ...
    if (l.includes('homepage') || l === 'home' || l === 'home page' || l === 'website') {
      return 3
    }
...
export class PyPiClient {
  ...
  async getPackageMetadata(packageName: string): Promise<{
    ...
    try {
      ...
      const summary: string | null =
        typeof info.summary === 'string' && info.summary.trim() ? info.summary.trim() : null
      ...
      const license: string | null =
        rawLicense && rawLicense !== 'UNKNOWN' && rawLicense !== 'NOASSERTION' ? rawLicense : null
  ...
  async findDocs(key: DocKey): Promise<HoverDoc | null> {
    const { url, summary, links, version, license, requiresPython } = await this.getPackageMetadata(
      key.package,
    )
    if (url || summary) {
      return {
