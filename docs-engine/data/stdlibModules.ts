...
export function buildStdlibDocsUrl(
  moduleName: string,
  pythonVersion: string,
): string | undefined {
  const normalized = moduleName.trim().replace(/^builtins\.?/, "");
  if (!normalized) {
    return undefined;
  }

  const exact = STDLIB_MODULES[normalized];
  if (exact) {
    return `https://docs.python.org/${pythonVersion}/${exact.url}`;
  }

  const lower = normalized.toLowerCase();
  const lowerExact = STDLIB_MODULES[lower];
  if (lowerExact) {
    return `https://docs.python.org/${pythonVersion}/${lowerExact.url}`;
  }

  // For dotted names (e.g. xml.etree.ElementTree.Element), walk parents
  // first so we prefer the closest known stdlib module page.
  if (lower.includes(".")) {
    const parts = lower.split(".");
    while (parts.length > 1) {
      parts.pop();
      const candidate = parts.join(".");
      const info = STDLIB_MODULES[candidate];
      if (info) {
        return `https://docs.python.org/${pythonVersion}/${info.url}`;
      }
    }

    return `https://docs.python.org/${pythonVersion}/library/${lower.split(".")[0]}.html`;
  }

  return `https://docs.python.org/${pythonVersion}/library/${lower}.html`;
}
