...
function printSample(title, doc) {
  ...
  console.log("-".repeat(72));
  console.log(
    "(Theme icons like $(book) render as icons in VS Code; here they are raw.)",
  );
}
...
printSample("Inventory-backed symbol provenance (Click)", {
  ...
  summary:
    "Declares a parameter that opens a file and converts it into a readable or writable stream.",
...
printSample("Method provenance (FastAPI.middleware)", {
  ...
  kind: "method",
  signature:
    "(self, middleware_type: str) -> Callable[[DecoratedCallable], DecoratedCallable]",
    ...
    sections: [
      {
        ...
        role: "summary",
        title: "Overview",
        content: "Parse a raw value into a structured result.",
      },
      {
        kind: "paragraph",
        title: "Description",
        content:
          "Use this helper when you want validated parsing with a predictable error surface.",
      },
      {
        kind: "list",
        title: "Parameters",
        content: "",
        items: [
          "value: Raw incoming value.",
          "strict: Raise on invalid input.",
        ],
      },
      {
        kind: "paragraph",
        title: "Returns",
        content: "Structured parse result.",
      },
      {
        kind: "paragraph",
        title: "Description",
