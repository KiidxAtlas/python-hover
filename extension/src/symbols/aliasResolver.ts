export class AliasResolver {
  resolve(documentText: string, symbol: string): string {
    ...
    if (variableAliases.has(root)) {
      ...
      const resolvedFromVariable =
        parts.length > 1 ? `${resolvedRoot}.${parts.slice(1).join('.')}` : resolvedRoot
  ...
  private parseAliases(text: string): {
    ...
    for (const line of lines) {
      ...
      const fromImportAsMatch = /^from\s+([\w.]+)\s+import\s+([\w.]+)\s+as\s+([\w.]+)$/.exec(
        trimmed,
      )
    ...
    for (const line of lines) {
      ...
      const assignmentMatch =
        /^([A-Za-z_][\w]*)\s*=\s*([A-Za-z_][\w]*(?:\.[A-Za-z_][\w]*)+)\s*\(/.exec(trimmed)
      if (!assignmentMatch) {
        continue
      }
