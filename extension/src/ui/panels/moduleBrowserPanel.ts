...
export class ModuleBrowserPanel {
  ...
  private moduleBrowserScriptUri(webview: vscode.Webview): string {
    return webview
      .asWebviewUri(
        vscode.Uri.joinPath(this.mediaRootUri(), "moduleBrowserWebview.js"),
      )
      .toString();
  }
  ...
  private buildInitialPackageOptions(symbols: IndexedSymbolSummary[]): string {
    const packages = [
      ...new Set(symbols.map((symbol) => symbol.package).filter(Boolean)),
    ].sort((left, right) => left.localeCompare(right));
    const options = [
      `<option value="all" selected>All packages (${packages.length})</option>`,
    ];
  ...
  private buildInitialSidebarMeta(symbols: IndexedSymbolSummary[]): string {
    const documentedCount = symbols.filter((symbol) =>
      this.hasInitialDocs(symbol),
    ).length;
  ...
  private buildInitialKindFilters(symbols: IndexedSymbolSummary[]): string {
    ...
    const chips = [
      `<span class="kind-chip active">All ${symbols.length}</span>`,
    ];
    for (const [kind, count] of [...counts.entries()].sort((left, right) =>
      left[0].localeCompare(right[0]),
    )) {
      chips.push(
        `<span class="kind-chip">${this.escapeHtml(kind)} ${count}</span>`,
      );
    }
  ...
  private buildInitialModuleActions(
    symbols: IndexedSymbolSummary[],
    moduleName: string,
  ): string {
  ...
  private buildInitialSelectedDetail(symbols: IndexedSymbolSummary[]): {
    ...
    const selected = [...symbols].sort((left, right) =>
          ...
          : rows
              .map((symbol) => {
                const summary = this.escapeHtml(
                  this.truncate(
                    symbol.summary ||
                      "No cached summary is available for this symbol yet.",
                    220,
                  ),
                );
  ...
  private hasInitialDocs(symbol: IndexedSymbolSummary): boolean {
    return Boolean(
      symbol.url || symbol.summary || symbol.signature || symbol.sourceUrl,
    );
  }

  private formatInitialRowMeta(symbol: IndexedSymbolSummary): string {
    const parts = [
      this.normalizeKind(symbol.kind),
      symbol.package || symbol.module || "module",
    ];
  ...
  private buildHtml(
    ...
    payload: ModuleBrowserPayload,
  ): string {
    const { moduleName, symbols, settings } = payload;
    const payloadData = this.serializeForScript({
      moduleName,
      symbols,
      settings,
    });
    ...
    const initialModuleActions = this.buildInitialModuleActions(
      symbols,
      moduleName,
    );
