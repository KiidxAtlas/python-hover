...
import { HOVER_PANEL_COMMANDS } from "#src/ui/webview/webviewCommandAllowlist";
...
export class HoverPanel {
  ...
  update(doc: HoverDoc): void {
    try {
      ...
      showErrorNotification(
        formatStandardErrorMessage("resolution-failed", doc.title),
        { logData: error },
      );
    }
  }
  ...
  private renderStructuredSection(section: StructuredHoverSection): string {
    ...
    if (section.kind === "list" && section.items && section.items.length > 0) {
      const items = section.items
        .map(
          (item) =>
            `<li>${this.markdownish(this.escape(cleanContent(item)))}</li>`,
        )
        .join("");
  ...
  private buildSourceHref(doc: HoverDoc): string | undefined {
    ...
    return this.buildCommandHref("python-hover.openHoverSource", {
      target: sourceTarget,
    });
  }
  ...
  private renderHeroActions(
    ...
    if (sourceHref) {
      primary.push(
        `<a class="chip chip-link" href="${this.escape(sourceHref)}">Source</a>`,
      );
    }
    ...
    secondary.push(
      `<a class="chip chip-link" href="command:python-hover.showHistory">History</a>`,
    );
    secondary.push(
      `<a class="chip chip-link" href="command:python-hover.openStudio">Studio</a>`,
    );
  ...
  private getCopyableSignature(doc: HoverDoc): string | undefined {
    return doc.signature
      ? buildCopyableSignature(doc.title, doc.signature)
      : undefined;
  }
  ...
  private renderNavigation(): string {
    ...
    const backHref =
      this.navigationIndex > 0
        ? this.buildCommandHref("python-hover.pinPanelBack")
        : "";
    const forwardHref =
      this.navigationIndex < this.navigationStack.length - 1
  ...
  private renderParameterLensCard(doc: HoverDoc): string {
    ...
    if (lens.parameter.default !== undefined) {
      chips.push(
        this.renderChip(`default ${lens.parameter.default}`, "neutral"),
      );
    }
  ...
  private renderHtml(doc: HoverDoc): string {
    ...
    if (doc.exportCount) {
      metaChips.push(
        this.renderChip(
          `${doc.exportCount.toLocaleString()} indexed`,
          "neutral",
        ),
      );
    }
    ...
    if (doc.badges) {
      for (const badge of doc.badges) {
        metaChips.push(
          this.renderChip(badge.label, this.getBadgeTone(badge.color)),
        );
      }
    }
    ...
    if (doc.signature) {
      ...
      signatureHtml = `<section class="card signature-card"><div class="section-kicker">Overloads</div><div class="signature-stack">${doc.overloads
        .map(
          (o) =>
            `<div class="signature-entry">${this.renderSignatureBody(cleanSignature(o))}</div>`,
        )
        .join("\n")}</div></section>`;
    }
    ...
    if (displayParameters.length > 0) {
      const rows = displayParameters
        .map((p, index) => {
          ...
          const name =
            p.default !== undefined
              ? `${e(p.name)}=${e(p.default)}`
              : e(p.name);
          const type = p.type ? e(p.type) : "—";
          const pdesc = p.description ? m(p.description) : "";
          const activeBadge = active
            ? '<span class="param-focus">Current argument</span>'
            : "";
          const description = [activeBadge, pdesc]
            .filter(Boolean)
            .join(pdesc ? " " : "");
...
<section class="card">
  ...
  <table>
    ...
    let returnsHtml = "";
    if (doc.returns) {
      const rtype = doc.returns.type
        ? `<code>${e(doc.returns.type)}</code> `
        : "";
  ...
  * { box-sizing: border-box; }
  ...
  .hero {
    background: linear-gradient(
      180deg,
      color-mix(in srgb, var(--panel-accent) 10%, var(--panel-surface-strong)),
      var(--panel-surface)
    );
...
<div class="shell">
  <header class="hero">
    ...
    ${this.renderNavigation()}
      ${actionLinks.primary.length > 0 ? `<div class="action-group"><div class="section-kicker">Quick actions</div><div class="hero-actions">${actionLinks.primary.join("")}</div></div>` : ""}
  </header>
  ${signatureHtml}
  ${parameterLensHtml}
  ${contentGridHtml}
