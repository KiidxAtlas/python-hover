...
import { RegularHoverSectionId } from '../hoverLayout'
...
export class HoverRenderer {
  ...
  private renderRegularHoverSections(
    ...
    for (const sectionId of order) {
      ...
      md.appendMarkdown(this.normalizeRegularHoverSectionFragment(fragment, !hasRenderedSection))
  ...
  private shouldRenderRegularHoverSection(
    ...
    switch (sectionId) {
      ...
      case 'parameters':
        return !compact && this.config.showParameters && (doc.parameters?.length ?? 0) > 0
      case 'returns':
        return !compact && this.config.showReturnTypes && !!doc.returns
      case 'raises':
        return !compact && this.config.showRaises && (doc.raises?.length ?? 0) > 0
      case 'examples':
        return (
          !compact &&
          this.config.showPracticalExamples &&
          ((doc.examples?.length ?? 0) > 0 || getStructuredExampleSections(doc).length > 0)
        )
      case 'seeAlso':
        return !compact && this.config.showSeeAlso && (doc.seeAlso?.length ?? 0) > 0
      case 'notes':
        return !compact && this.config.showNotes && getVisibleStructuredNoteSections(doc).length > 0
      case 'footer':
        return (
          this.config.showFooter && doc.kind !== 'module' && doc.kind?.toLowerCase() !== 'keyword'
        )
    }
  }
  ...
  private normalizeRegularHoverSectionFragment(fragment: string, isFirst: boolean): string {
    if (!isFirst) {
      return fragment
    }
  ...
  private renderSignature(md: vscode.MarkdownString, doc: HoverDoc): void {
    if (doc.overloads && doc.overloads.length > 1) {
      ...
      doc.overloads
        .slice(0, maxShow)
        .forEach(o => this.renderSignatureEntry(md, this.normalizeDisplaySignature(o), true))
  ...
  private renderDescription(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    if (doc.kind?.toLowerCase() === 'keyword') {
      // If structuredContent already has a code/syntax section (pre-extracted by the corpus),
      // use the structured path so the grammar block renders as a proper code block.
      if (doc.structuredContent?.sections?.some(s => s.kind === 'code')) {
        md.appendMarkdown(`**$(book) Overview**\n\n`)
        const sections = doc.structuredContent.sections
        // Print the summary title if the first section has one
        if (sections[0]?.title) {
          md.appendMarkdown(`**${sections[0].title}**\n\n`)
        }
        for (const section of sections) {
          if (section.kind === 'code') {
            const label = section.title ?? 'Syntax'
            md.appendMarkdown(`---\n\n**$(code) ${label}**\n\n`)
            // Always use 'python' for keyword grammar (corpus stores it as 'text')
            md.appendCodeblock(section.content, 'python')
            md.appendMarkdown('\n')
          } else if (section.role !== 'summary' || !section.title) {
            // For summary sections the title was already printed; for others render content
            const text = this.formatKeywordParagraph(section.content)
            if (text) {
      ...
      // Fall back to pydoc parser when no structured sections available.
  ...
  private renderStructuredSection(section: StructuredHoverSection): string {
    ...
    if (section.kind === 'list') {
      const items = (section.items ?? [])
        .map(item =>
          this.enhanceContent(this.cleanContentAnnotations(this.cleanRstArtifacts(item)).trim()),
        )
        .filter(Boolean)
      if (items.length === 0) {
        return ''
  ...
  private renderKeywordContent(md: vscode.MarkdownString, content: string, doc: HoverDoc): void {
    ...
    for (const block of parsed.blocks) {
      if (block.kind === 'syntax') {
        syntaxBlocks.push(block.content)
        continue
      }
      ...
      if (inlineExtracted) {
        if (inlineExtracted.overviewPrefix) {
          const prefixText = this.formatKeywordParagraph(inlineExtracted.overviewPrefix)
          if (prefixText) {
            overviewBlocks.push(this.rewriteMarkdownLinks(prefixText))
          }
        }
        syntaxBlocks.push(inlineExtracted.syntax)
        if (inlineExtracted.overviewSuffix) {
          const suffixText = this.formatKeywordParagraph(inlineExtracted.overviewSuffix)
          if (suffixText) {
    ...
    if (syntaxBlocks.length > 0) {
      ...
      syntaxBlocks.forEach((syntax, index) => {
        ...
        if (lines.length > maxSyntaxLines) {
          md.appendCodeblock(lines.slice(0, maxSyntaxLines).join('\n'), 'python')
          md.appendMarkdown(`*+${lines.length - maxSyntaxLines} more grammar lines in docs*\n\n`)
        } else {
  ...
  private parseKeywordContent(content: string): {
    ...
    while (index < lines.length) {
      ...
      if (/^(?:See also|Related help topics?):?\s*/i.test(trimmed)) {
        ...
        if (related) {
          const parts = related.includes(',') ? related.split(',') : related.split(/\s+/)
  ...
  private isKeywordSyntaxStart(line: string): boolean {
    return (
      line.includes('::=') || /^`?[a-z][a-z0-9_]*(?:_stmt|_expr|_clause|_list)?`?\s*:\s*/.test(line)
    )
  }

  private extractInlineKeywordSyntax(
    paragraph: string,
  ): { overviewPrefix?: string; syntax: string; overviewSuffix?: string } | null {
    // Match grammar-like rule starts both plain and backticked:
    //   if_stmt: ...
    //   `if_stmt`: ...
    const ruleStart = /(?:^|\s)(`?[a-z][a-z0-9_]*(?:_[a-z0-9_]+)*`?\s*:)/
    const match = ruleStart.exec(paragraph)
    ...
    if (!/[_]|::=/.test(fragment) && !/^`?[a-z][a-z0-9_]*`?\s*:/.test(fragment)) {
      return null
    }
    ...
    for (const line of lines) {
      const looksRule =
        /^`?[a-z][a-z0-9_]*(?:_[a-z0-9_]+)*`?\s*:/.test(line) || line.includes('::=')
      if (!inSuffix && looksRule) {
        syntaxLines.push(line.replace(/^`|`(?=\s*:)/g, ''))
        continue
      }
      inSuffix = true
  ...
  private isKeywordSyntaxBlock(block: string): boolean {
    ...
    return lines.every(
      line => /^[()[\]{}|"'\s\w:.*,+-]+$/.test(line) && !this.looksLikePythonCode(line),
    )
  }
  ...
  private looksLikePythonCode(line: string): boolean {
    ...
    if (
      /^(?:try:|except\b.*:|finally:|assert\s|pass$|break$|continue$|del\s|lambda\b)/.test(line)
    ) {
      return true
    }
  ...
  private renderKeywordSeeAlso(md: vscode.MarkdownString, raw: string, doc: HoverDoc): void {
    ...
    const peps = [...normalized.matchAll(/\*{0,2}PEP\s*(\d+)\*{0,2}/gi)].map(match =>
    ...
    if (normalized) {
      ...
      if (tokens.length > 0 && tokens.every(token => /^[A-Za-z_][A-Za-z0-9_.]*$/.test(token))) {
        parts.push(
          tokens.map(token => this.renderInlineReferenceItem(token, doc)).join(' \u00a0·\u00a0 '),
        )
      } else {
  ...
  private renderParameterLens(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    const description = lens.parameter.description
      ? this.rewriteMarkdownLinks(
          this.enhanceContent(
            this.cleanContentAnnotations(this.cleanRstArtifacts(lens.parameter.description))
              .replace(/\s+/g, ' ')
              .trim(),
          ),
        )
  ...
  private renderModuleOverview(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    if (
      ...
      const preview = doc.moduleExports
        ...
        .join(' \u00a0·\u00a0 ')
      const suffix =
        doc.moduleExports.length > 4 ? ` \u00a0·\u00a0 +${doc.moduleExports.length - 4} more` : ''
  ...
  private renderReturns(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    const cleanedDescription = ret.description
      ? this.rewriteMarkdownLinks(
          this.enhanceContent(
            this.cleanContentAnnotations(this.cleanRstArtifacts(ret.description))
              .replace(/\s+/g, ' ')
              .trim(),
          ),
        )
  ...
  private renderRaises(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    doc.raises!.forEach(exc => {
      const cleanedDescription = exc.description
        ? this.rewriteMarkdownLinks(
            this.enhanceContent(
              this.cleanContentAnnotations(this.cleanRstArtifacts(exc.description))
                .replace(/\s+/g, ' ')
                .trim(),
            ),
          )
  ...
  private renderModuleExports(md: vscode.MarkdownString, doc: HoverDoc): void {
    ...
    md.appendMarkdown(shown.map(name => `\`${name}\``).join(' \u00a0 ') + '\n\n')
  ...
  private buildActionGroups(doc: HoverDoc): {
    ...
    primaryActions.push(
      this.buildCommandLink('$(pin) Pin', 'python-hover.pinHover', commandToken, 'Pin this hover'),
    )
  ...
  private getCopyableSignature(doc: HoverDoc): string | undefined {
    return doc.signature
      ? buildCopyableSignature(doc.title, this.normalizeDisplaySignature(doc.signature))
      : undefined
  }
  ...
  private rewriteMarkdownLinks(content: string): string {
    return content.replace(
      ...
      (_match, label: string, rawUrl: string) => {
        const linked = this.buildLinkUrl(rawUrl, this.config.docsBrowser, 'docs')
        return `[${label}](<${linked}>)`
      },
    )
  }
