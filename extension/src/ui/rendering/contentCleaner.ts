...
export function cleanPydocDump(text: string): string {
  ...
  if (
    /^class\s+\w+.*\|/s.test(text) ||
    /\|\s+Methods defined here:/s.test(text)
  ) {
...
export function cleanRstArtifacts(text: string): string {
  ...
  text = text.replace(
    /:(?:func|class|meth|mod|attr|exc|data|const|type|obj):`([^`]+)`/g,
    "`$1`",
  );
  text = text.replace(/\|([^|\n]+)\|_?/g, "$1");
...
export function cleanContentAnnotations(text: string): string {
  ...
  text = text.replace(/\n{3,}/g, "\n\n");
  // Final sanitization: strip tags from any reconstructed HTML fragments.
  text = stripDangerousTagBlocks(text, ["script", "style"]);
...
export function cleanSignature(sig: string): string {
  ...
  sig = sig.replace(
    /<_py_warnings\.deprecated\s+object\s+at\s+0x[0-9a-f]+>/gi,
    "",
  );
...
export function cleanContent(text: string): string {
  ...
  if (!text.trim()) {
    Logger.debug("contentCleaner: cleaned text to empty", {
...
function normalizeSphinxSignatureArtifacts(text: string): string {
  return text.replace(
    ...
    (_match, keyword, name, params) =>
      `${keyword} ${name}${normalizeSphinxSignatureParams(params)}`,
  );
}

function normalizeSphinxSignatureParams(params: string): string {
  return params.replace(/\*([^*,()]+)\*/g, "$1").replace(/\*{2,}/g, "*");
}
...
function isLegalBoilerplateBlock(block: string): boolean {
  ...
  if (
    LEGAL_BLOCK_OPENING_PATTERNS.some((pattern) => pattern.test(normalized))
  ) {
    return true;
  }
