...
export class SavedDocsView extends BaseTreeView<SavedDocEntry> {
  ...
  getTreeItem(element: SavedDocsTreeNode): vscode.TreeItem {
    ...
    item.id = element.id
    item.description = this.buildMetaDescription(
      this.formatKindLabel(element.kind),
      element.module ?? element.package,
    )
  ...
  private buildTooltip(entry: SavedDocEntry): vscode.MarkdownString {
    const meta = this.buildMetaDescription(
      this.formatKindLabel(entry.kind),
