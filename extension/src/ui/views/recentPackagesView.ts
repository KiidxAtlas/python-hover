...
import { BaseTreeView, PlaceholderTreeNode } from '#src/ui/views/baseTreeView'
...
export class RecentPackagesView extends BaseTreeView<RecentPackageEntry> {
  ...
  getTreeItem(element: RecentPackagesTreeNode): vscode.TreeItem {
    if (this.isPlaceholderNode(element)) {
      return this.buildPlaceholderTreeItem(
        element,
        'symbol-namespace',
        'pyhoverRecentPackagesPlaceholder',
      )
    }
    ...
    item.tooltip = this.buildMarkdownTooltip(
      element.name,
      `${element.count.toLocaleString()} indexed symbols`,
      'Click to open the module browser.',
    )
