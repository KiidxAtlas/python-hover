...
import { buildSavedDocEntry } from '#src/state/savedDocs'
import { cleanContent, cleanSignature } from '#src/ui/rendering/contentCleaner'
import {
...
export class HoverInspectorView implements vscode.TreeDataProvider<InspectorNode> {
  ...
  private buildEmptyNodes(): InspectorNode[] {
    return [
      ...
      this.commandNode(
        'empty-studio',
        'Open Studio',
        'Settings and cache tools',
        'layout',
        'python-hover.openStudio',
      ),
      this.commandNode(
        'empty-history',
        'Hover history',
        'Recent hover targets',
        'history',
  ...
  private buildDocNodes(doc: HoverDoc): InspectorNode[] {
    ...
    nodes.push({
      ...
      tooltip: this.markdownTooltip(
        `**${this.escapeMarkdown(title)}**${subtitle ? `\n\n${this.escapeMarkdown(subtitle)}` : ''}`,
      ),
    ...
    if (doc.parameterLens) {
      ...
      const lensChildren: InspectorNode[] = [
        this.infoNode(
          'lens-parameter',
          'Parameter',
          `${lens.parameter.name || lens.parameterLabel} (${lens.parameterIndex + 1}/${lens.parameterCount})`,
          'target',
        ),
      ]
      if (lens.parameter.type) {
        lensChildren.push(
          this.infoNode('lens-type', 'Type', lens.parameter.type, 'symbol-parameter'),
        )
      }
      if (lens.parameter.default !== undefined) {
        lensChildren.push(
          this.infoNode('lens-default', 'Default', lens.parameter.default, 'symbol-value'),
        )
      }
      ...
      if (!currentSignature || currentSignature !== lensSignature) {
        lensChildren.push(
          this.infoNode(
            'lens-signature',
            'Call',
            this.clipText(lensSignature, 80),
            'symbol-method',
          ),
        )
      }
    ...
    actionChildren.push(
      this.commandNode(
        ...
        undefined,
        'pin',
        token ? 'python-hover.pinHover' : 'python-hover.pinLast',
    ...
    if (doc.sourceUrl || doc.links?.source) {
      actionChildren.push(
        this.commandNode(
          ...
          [{ token, target: doc.sourceUrl || doc.links?.source }],
        ),
      )
    }
    ...
    if (doc.module) {
      contextChildren.push(
        this.infoNode('context-module', 'Module', doc.module, 'symbol-namespace'),
      )
    }
    const packageName =
    ...
    contextChildren.push(
      this.infoNode('context-source', 'Source', this.renderSourceLabel(doc), 'server-environment'),
    )
    if (doc.installedVersion) {
      contextChildren.push(
        this.infoNode('context-version', 'Installed', doc.installedVersion, 'tag'),
      )
    }
    ...
    if (doc.requiresPython) {
      contextChildren.push(
        this.infoNode('context-python', 'Python', doc.requiresPython, 'versions'),
      )
    }
    if (importStatement) {
      contextChildren.push(
        this.infoNode('context-import', 'Import', importStatement, 'symbol-file'),
      )
    }
    if (doc.exportCount) {
      contextChildren.push(
        this.infoNode(
          'context-indexed',
          'Indexed',
          `${doc.exportCount.toLocaleString()} symbols`,
          'list-unordered',
        ),
      )
    }
    ...
    if (example) {
      nodes.push(
        this.leafNode(
          'example',
          'Example',
          this.clipText(example.replace(/\s+/g, ' '), 110),
          'code',
          example,
        ),
      )
    ...
    if (doc.parameters && doc.parameterLens) {
      const activeParameter = doc.parameters.find((parameter, index) =>
        isActiveParameterMatch(doc.parameterLens, parameter, index),
      )
      if (activeParameter?.description) {
        relatedChildren.push(
          this.infoNode(
            'related-active-parameter',
            'Current arg',
            this.clipText(cleanContent(activeParameter.description), 110),
            'target',
          ),
        )
      }
    ...
    if (doc.returns?.type || doc.returns?.description) {
      const returnsDetail = [
        doc.returns.type,
        doc.returns.description ? cleanContent(doc.returns.description) : undefined,
      ]
        .filter(Boolean)
        .join(' — ')
      relatedChildren.push(
        this.infoNode('related-returns', 'Returns', this.clipText(returnsDetail, 110), 'return'),
      )
    }
    ...
    if (
      ...
      availabilityChildren.push(
        this.infoNode(
          'availability-update',
          'Update',
          `${doc.installedVersion} -> ${doc.latestVersion}`,
          'arrow-up',
        ),
      )
    }
    ...
    if (requiredVersion) {
      availabilityChildren.push(
        this.infoNode('availability-python', 'Requires', `Python ${requiredVersion}+`, 'warning'),
      )
    }
  ...
  private buildModuleSymbolsSection(
    ...
    const children = visibleSymbols.map((symbol, index) =>
      this.buildModuleSymbolNode(symbol, index),
    )
  ...
  private buildModuleSymbolNode(symbol: IndexedSymbolSummary, index: number): InspectorNode {
    ...
    return {
      ...
      tooltip: this.markdownTooltip(
        tooltipParts.map(part => this.escapeMarkdown(part)).join('\n\n'),
      ),
      command: {
