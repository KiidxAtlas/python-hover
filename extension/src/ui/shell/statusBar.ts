...
export class StatusBarManager {
  ...
  private render() {
    ...
    tt.appendText('Click for quick actions like search, browse, hover history, cache tools, and Studio.')
  ...
  public async showStatusNotification(): Promise<void> {
    ...
    const formatKind = (kind?: string) =>
      kind ? `${kind.charAt(0).toUpperCase()}${kind.slice(1).toLowerCase()}` : undefined
      ...
      ...(recentHistory.length > 0
        ? [
            ...
            ...recentHistory.map(entry => ({
              ...
              run: () =>
                vscode.commands.executeCommand('python-hover.openSidebarHistoryEntry', entry),
            })),
          ]
        : []),
      ...(savedDocs.length > 0
  ...
  public async toggleOnlineDiscovery(): Promise<void> {
    ...
    vscode.window.showInformationMessage(`PyHover: online discovery ${!cur ? 'enabled' : 'disabled'}`)
  }
