...
export class LspClient {
  ...
  private readonly resolvedSymbolCache = new Map<
    string,
    { expiresAt: number; value: LspSymbol | null }
  >()
  private readonly inflightResolvedSymbols = new Map<string, Promise<LspSymbol | null>>()
  private readonly documentSymbolCache = new Map<
    string,
    { expiresAt: number; value: vscode.DocumentSymbol[] | undefined }
  >()
  private readonly inflightDocumentSymbols = new Map<
    string,
    Promise<vscode.DocumentSymbol[] | undefined>
  >()
  private readonly hoverQueryCache = new Map<
    string,
    { expiresAt: number; value: vscode.Hover[] | undefined }
  >()
  ...
  async resolveSymbol(
    ...
    try {
      ...
      this.setCacheEntry(
        this.resolvedSymbolCache,
        cacheKey,
        this.cloneSymbol(result),
        RESOLVED_SYMBOL_CACHE_TTL_MS,
      )
  ...
  private async resolveSymbolUncached(
    ...
    const queryColumn = expression.includes('.') ? queryChar : segmentRange.start.character
    ...
    const [definitions, hovers] = await Promise.all([
      ...
      this.executeExternalHoverQuery(document.uri, queryPos),
    ])
    ...
    this.preserveStdlibQualifiedExpression(result, expression)
  ...
  private buildContext(
    ...
    const rightMatch = /^\.([A-Za-z_][A-Za-z0-9_]*)/.exec(line.slice(segmentRange.end.character))
  ...
  private async hydrateFromDefinitionHover(
    ...
    const hovers = await this.executeExternalHoverQuery(loc.uri, loc.range.start)
  ...
  private preserveStdlibQualifiedExpression(result: LspSymbol, expression: string): void {
    if (!expression.includes('.')) {
      return
    }

    const root = expression.split('.')[0]
    if (!isStdlibTopLevelModule(root)) {
      return
    }

    const currentName = result.name || ''
    const currentLeaf = currentName.split('.').pop() || currentName
    const expressionLeaf = expression.split('.').pop() || expression

    if (currentName.includes('.') && currentName !== currentLeaf) {
      return
    }

    if (currentLeaf !== expressionLeaf || currentName === currentLeaf) {
      result.name = expression
      if (!result.module || result.module === 'builtins') {
        result.module = expression.slice(0, expression.lastIndexOf('.'))
      }
    }
  }
  ...
  private async executeExternalHoverQuery(
    ...
    const query = this.lspQuery<vscode.Hover[]>('vscode.executeHoverProvider', uri, position)
  ...
  private async resolveSymbolAtLocation(loc: vscode.Location): Promise<string | null> {
    try {
      ...
      const findPath = (list: vscode.DocumentSymbol[]): string[] | null => {
        for (const s of list) {
          if (!s.range.contains(loc.range.start)) {
            continue
          }
  ...
  private moduleFromPath(fsPath: string): string | null {
    ...
    for (const marker of markers) {
      ...
      if (['ntpath', 'posixpath', 'macpath'].includes(mod)) {
        mod = 'os.path'
      }
