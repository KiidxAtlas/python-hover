import { HoverHistoryEntry, IndexedSymbolSummary, SavedDocEntry } from '#shared/types'
import {
    buildSearchResultDescription,
    buildSearchResultDetail,
    formatSymbolKindLabel,
    iconForSymbolKind,
} from '#src/runtime/browser/searchPresentation'
...
export function registerSearchDocsCommand(
  ...
  return () => {
      ...
      const buildStarterSearchItems = (): SearchItem[] => {
        ...
        const recentPackageItems = deps.getRecentPackageSummaries(6).map(pkg => ({
