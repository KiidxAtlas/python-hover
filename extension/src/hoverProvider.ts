...
export class HoverProvider implements vscode.HoverProvider {
  ...
  async getIndexedSymbolPreviews(symbols: IndexedSymbolSummary[]): Promise<IndexedSymbolPreview[]> {
    const uniqueSymbols = [
      ...new Map(symbols.map(symbol => [`${symbol.package}:${symbol.name}`, symbol])).values(),
    ]
  ...
  private buildIndexedSymbolInfo(symbol: IndexedSymbolSummary): SymbolInfo {
    ...
    const module =
      symbol.module || (parts.length > 1 ? parts.slice(0, -1).join('.') : symbol.package || name)
    return {
      name,
      qualname: name,
      kind: symbol.kind,
      module,
      isStdlib:
        symbol.package === 'builtins' || isStdlibTopLevelModule(symbol.package || parts[0] || ''),
    }
  }
  ...
  private addDeprecatedDiagnostic(
    ...
    const alreadyTracked = existing.some(
      diag => diag.code === 'deprecated' && diag.message === message && diag.range.isEqual(range),
    )
    if (alreadyTracked) {
      return
    }
    ...
    try {
      ...
      const hoverPromise = (async () => {
        ...
        const shouldTrustImportedOrStdlibRoot =
          !!dottedRoot && (importedRoots.has(dottedRoot) || isStdlibTopLevelModule(dottedRoot))
        ...
        const needsAst =
          ...
          (canAst &&
            !!lspSymbol &&
            (!lspSymbol.path || !lspSymbol.name.includes('.') || hasUserDefinitionPath))
  ...
  private async resolveAndRender(
    ...
    try {
      ...
      const inferredReceiverOwner = shouldInferReceiverOwner
        ...
        : null
      const builtinOwner =
        this.inferBuiltinOwnerFromSignature(symbolInfo.signature) ?? inferredReceiverOwner
      if (builtinOwner) {
      ...
      if (
        (symbolInfo.module === 'typing' || symbolInfo.name?.startsWith('typing.')) &&
      ...
      const isUnimportedThirdPartyGuess =
        !hasTrustedLibraryEvidence && !docKey.isStdlib && !!rootName && docKey.package === rootName
      ...
      const isThirdParty =
        !symbolInfo.isStdlib && !!symbolInfo.module && symbolInfo.module !== 'builtins'
      const topModule = symbolInfo.module?.split('.')[0] ?? ''

      const ivKey = `${this.envCacheId}:${topModule}`
      const installedVersionPromise =
      ...
      if (installedVersion) {
        hoverDoc.installedVersion = installedVersion
      }
      ...
      if (
        ...
        // Nothing to display — suppress the hover entirely so we don't show a
        // confusing header-only card.  Cache as negative so the pipeline is not
        // re-entered for every cursor movement over the same symbol.
        this.cacheNegativeHover(cacheKey)
        this.logHoverEvent(
          `resolved-empty:${cacheKey}:${symbolInfo.name}`,
          `Hover: resolved ${symbolInfo.name} but found no displayable documentation.`,
        )
        return null
      ...
      if (
        ...
        hoverDoc.badges?.some(b => b.label === 'deprecated') &&
  ...
  private isWeakLibrarySignature(signature?: string): boolean {
    ...
    return (
      /^\(\*args,?\s*\*\*(?:kwargs|kwds)\)$/.test(normalized) || /^\(\.\.\.\)$/.test(normalized)
    )
  }
  ...
  private async resolveKeyword(
    ...
    const content = resolvedDoc?.content || runtimeInfo?.docstring || resolvedDoc?.summary
    if (!content) {
      return null
    }

    // Attribute source to whichever data origin actually supplied the content.
    const contentFromRuntime = !!runtimeInfo?.docstring && content === runtimeInfo.docstring
    ...
    const keywordDoc: HoverDoc = {
      ...
      kind: 'keyword',
      content,
      structuredContent: !contentFromRuntime ? resolvedDoc?.structuredContent : undefined,
  ...
  private async inferBuiltinOwnerFromReceiver(
    ...
    const receiverSymbol = await this.lspClient
      .resolveSymbol(document, receiverPosition)
      .catch(() => null)
  ...
  private inferBuiltinOwnerFromLiteralReceiver(
    ...
    if (
      /(?:\d[\d_]*\.\d[\d_]*|\d[\d_]*[eE][+-]?\d[\d_]*|\.\d[\d_]+)(?:[jJ])?\s*$/.test(literalText)
    ) {
  ...
  private findBalancedOpeningBracket(
    line: string,
    closeIndex: number,
    openChar: string,
    closeChar: string,
