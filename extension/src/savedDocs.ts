...
export function getSavedDocEntryId(
  entry: Pick<SavedDocEntry, 'title' | 'kind' | 'module' | 'url' | 'sourceUrl'>,
): string {
  return [
...
export function normalizeSavedDocEntry(
  entry: Partial<SavedDocEntry> | undefined,
...
export function buildSavedDocEntry(doc: HoverDoc): SavedDocEntry | undefined {
  ...
  const packageName =
    typeof doc.metadata?.indexedPackage === 'string'
      ...
      : moduleName?.split('.')[0]
  const sourceUrl =
    doc.sourceUrl || (typeof doc.links?.source === 'string' ? doc.links.source : undefined)
  const summary = doc.summary?.trim() || doc.structuredContent?.summary?.trim() || undefined
  const commandToken =
    typeof doc.metadata?.commandToken === 'string' ? doc.metadata.commandToken : undefined
