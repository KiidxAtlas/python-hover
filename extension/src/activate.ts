...
import { setEngineLogger } from '../../docs-engine/src/engineLogger'
import {
  HoverDoc,
  HoverHistoryEntry,
...
import { normalizeRegularHoverSectionOrder, RegularHoverSectionId } from './hoverLayout'
import { HoverProvider } from './hoverProvider'
import {
  openHoverDocSource,
  openIndexedSymbolSource,
  openSourceTarget,
} from './indexedSymbolActions'
  ...
  try {
    ...
    const updateContextMenuContexts = () => {
      void Promise.all([
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.enabled',
          config.showEditorContextMenu,
        ),
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.searchDocs',
          config.showSearchDocsContextMenu,
        ),
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.browseModule',
          config.showBrowseModuleContextMenu,
        ),
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.pinHover',
          config.showPinHoverContextMenu,
        ),
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.debugPinHover',
          config.showDebugPinHoverContextMenu,
        ),
        vscode.commands.executeCommand(
          'setContext',
          'pyhover.contextMenu.openStudio',
          config.showOpenStudioContextMenu,
        ),
      ])
    }
    ...
    context.subscriptions.push(
      vscode.workspace.onDidChangeConfiguration(event => {
        ...
        const affectsDiskCache =
          affectsInterpreter || event.affectsConfiguration('python-hover.cacheTTL')
        ...
        if (affectsDiskCache || affectsProviderCore) {
          rebuildHoverRuntime('Configuration changed. Recreating hover provider.', affectsDiskCache)
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('python-hover.copyUrl', async (urlOrToken?: string) => {
        const explicitUrl =
          typeof urlOrToken === 'string' && /^(?:https?:|file:|\/|[A-Za-z]:\\)/.test(urlOrToken)
            ? urlOrToken
            : undefined
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('python-hover.copySignature', async (sigOrToken?: string) => {
        const explicitSignature =
          typeof sigOrToken === 'string' && (sigOrToken.includes('(') || sigOrToken.includes('->'))
            ? sigOrToken
            : undefined
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('python-hover.clearCache', () => {
        ...
        vscode.window.showInformationMessage(
          'PyHover cache cleared. Python stdlib corpus preserved.',
        )
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('python-hover.clearAllCache', () => {
        ...
        vscode.window.showInformationMessage(
          'PyHover cleared all cached docs, inventories, and stdlib corpus data.',
        )
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('python-hover.cancelPythonCorpusBuild', () => {
        if (!activeCorpusBuild) {
          vscode.window.showInformationMessage(
            'No Python stdlib corpus build is currently running.',
          )
          return
        }
      ...
      vscode.commands.registerCommand('python-hover.buildPythonCorpus', async () => {
        if (!config.onlineDiscovery) {
          vscode.window.showWarningMessage(
            'Enable python-hover.onlineDiscovery to build the Python corpus.',
          )
          return
        }
        ...
        try {
          const result = await vscode.window.withProgress(
            ...
            async (progress, token) => {
              ...
              try {
                let lastReported = 0
                return await hoverProvider.buildPythonCorpus(
        ...
        const buildStarterSearchItems = (): SearchItem[] => {
          const savedDocItems = getSavedDocs()
            ...
            .map(entry => ({
              ...
              description: [formatSymbolKindLabel(entry.kind), entry.module ?? entry.package]
                .filter(Boolean)
                .join(' • '),
          ...
          const historyItems = hoverProvider
            .getHoverHistory()
          ...
          switch (message.type) {
            ...
            case 'open-settings':
              await vscode.commands.executeCommand(
                ...
                isAllowedSettingsQuery(message.query, 'python-hover.ui.moduleBrowser'),
              )
              break
            case 'update-setting':
    ...
    const resolveSavedDocEntry = (
      payload?: string | Partial<SavedDocEntry>,
    ): SavedDocEntry | undefined => {
      if (typeof payload === 'string') {
    ...
    const getRecentPackages = (): string[] => {
      ...
      const fromHistory = hoverProvider
        .getHoverHistory()
    ...
    const getRecentPackageSummaries = (limit = 6): Array<{ name: string; count: number }> => {
      const counts = new Map(
        hoverProvider.getIndexedPackageSummaries().map(summary => [summary.name, summary.count]),
      )
    ...
    const getCurrentStudioSettingValues = (): Map<string, boolean | string | number> =>
      new Map<string, boolean | string | number>([
    ...
    const reorderHoverSection = async (
      ...
      const current = normalizeRegularHoverSectionOrder(config.hoverSectionOrder)
      ...
      await updateSettingWithPreferredTarget('python-hover', 'ui.hoverSectionOrder', next)
    }
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        ...
        async (symbol: IndexedSymbolSummary) => {
          ...
          const opened = await openIndexedSymbolSource(symbol, hoverProvider, url =>
