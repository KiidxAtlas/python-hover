...
export class Config {
  ...
  get customLibraries(): CustomLibraryConfig[] {
    ...
    return raw.flatMap(entry => {
      ...
      const inventoryUrl =
        typeof candidate.inventoryUrl === 'string' ? candidate.inventoryUrl.trim() : undefined
      if (!name || !baseUrl) {
        return []
      }
  ...
  get pythonPath(): string {
    ...
    const configured =
      pythonConfig.get<string>('defaultInterpreterPath') || pythonConfig.get<string>('pythonPath')
  ...
  get interpreterCacheFingerprint(): string {
    ...
    if (this._fingerprintCache?.path === p) {
      return this._fingerprintCache.fingerprint
    }
