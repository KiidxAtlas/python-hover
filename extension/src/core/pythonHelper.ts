...
type GetDocstringRequest = {
  cmd: "get_docstring";
  source: string;
  symbol: string;
};
type ResolveSourceSymbolRequest = {
...
type PythonCommandFor<T extends PythonCommandName> = Extract<
  PythonServerCommand,
  { cmd: T }
>;
...
class PythonProcess {
  ...
  start(): void {
    ...
    try {
      ...
      this.proc.stderr!.on("data", (data: Buffer) => {
        ...
        if (msg) {
          Logger.log(`[PythonProcess stderr] ${msg}`);
        }
      });
  ...
  send<T extends PythonCommandName>(
    cmd: PythonCommandFor<T>,
    timeoutMs: number,
  ): Promise<PythonCommandResultMap[T]> {
...
export class PythonHelper {
  ...
  constructor(
    ...
    // (previously used 3 levels which resolved into out/python-helper)
  ...
  private isPathLikeAbsolute(p: string): boolean {
    return (
      path.isAbsolute(p) ||
      this.looksLikeWindowsPath(p) ||
      this.looksLikePosixPath(p)
    );
  }
  ...
  private async ensurePythonPathResolved(): Promise<string | null> {
    ...
    this.resolvedPath = (async () => {
      ...
      for (const candidate of candidates) {
        ...
        if (await this.canRunInterpreter(candidate)) {
          if (candidate !== this.pythonPath) {
            Logger.log(
              `PythonHelper: using fallback interpreter: ${candidate}`,
            );
          }
      ...
      Logger.error(
        `PythonHelper: no usable Python interpreter. Tried: ${candidates.join(", ")}`,
      );
      return null;
    })();
  ...
  private async send<T extends PythonCommandName>(
    cmd: PythonCommandFor<T>,
    timeoutMs = 3000,
  ): Promise<PythonCommandResultMap[T] | null> {
  ...
  async resolveRuntime(symbol: string): Promise<SymbolInfo | null> {
    ...
    if (cached) {
      try {
        ...
        Logger.error(
          `PythonHelper: failed to parse cached data for ${symbol}`,
          e,
        );
      }
    }

    // If persistent runtime disabled, fall back to a one-shot helper invocation
    if (!this.enablePersistentRuntime) {
      Logger.log(`PythonHelper: resolving ${symbol} via one-shot helper`);
      ...
      try {
        const info = await new Promise<SymbolInfo | null>((resolve) => {
          let out = "";
          let settled = false;
          const proc = cp.spawn(exe, [this.helperPath, "--resolve", symbol], {
            stdio: ["ignore", "pipe", "ignore"],
          });
          ...
          proc.on("close", (code) => {
            ...
            try {
              ...
              const si: SymbolInfo = {
                ...
                docstring: (result as any).docstring ?? undefined,
                signature: (result as any).signature ?? undefined,
                path: undefined,
                isStdlib: (result as any).is_stdlib,
                qualname: (result as any).qualname,
        ...
        Logger.error("PythonHelper one-shot resolve failed", e);
        this.sessionCache.set(symbol, null);
        return null;
      }
    }
    ...
    if (!result || this.isErrorResult(result)) {
      Logger.log(
        `PythonHelper: resolve failed for ${symbol}: ${result?.error}`,
      );
      if (this.sessionCache.size >= PythonHelper.SESSION_CACHE_MAX) {
        const oldest = this.sessionCache.keys().next().value;
        if (oldest !== undefined) {
  ...
  async identify(
    ...
    const result = await this.send(
      { cmd: "identify", source, line, col: column },
      1000,
    );
  ...
  async getLocalDocstring(
    ...
    const result = await this.send(
      { cmd: "get_docstring", source, symbol },
      2000,
    );
    if (!result || this.isErrorResult(result)) {
      return null;
  ...
  async getInstalledVersion(packageName: string): Promise<string | null> {
    ...
    const result = await this.send(
      { cmd: "pkg_version", package: packageName },
      1500,
    );
