...
export function getHoverDocSourceTarget(
  doc: Pick<HoverDoc, 'sourceUrl' | 'links' | 'url'>,
): string | undefined {
...
export async function openSourceTarget(
  ...
  const uri = sourceTarget.includes('://')
    ? vscode.Uri.parse(sourceTarget)
    : vscode.Uri.file(sourceTarget)
  try {
...
async function findWorkspaceSymbolLocation(
  ...
  for (const query of queries) {
    const result = await vscode.commands.executeCommand<WorkspaceSymbolCandidate[]>(
  ...
  const ranked = candidates
    .map(candidate => ({
      candidate,
      score: scoreWorkspaceSymbolCandidate(candidate.name || '', target, leaf),
    }))
...
function scoreWorkspaceSymbolCandidate(
