import { CacheCommandsDeps, createCacheCommands } from '#src/commands/cacheCommands'
...
type RegisterRuntimeCommandsDeps = {
  ...
  getRecentPackageSummaries: (limit?: number) => Array<{ name: string; count: number }>
...
export function registerRuntimeCommands(deps: RegisterRuntimeCommandsDeps): void {
  const runSearchDocs = registerSearchDocsCommand({
    ...
    rememberRecentPackage: packageName => deps.rememberRecentPackage(packageName),
  ...
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.toggleLearnMode', async () => {
      const enabled = !deps.getLearnModeEnabled()
      await deps.updateSetting('ui.learnMode', enabled)
      if (enabled) {
        await deps.updateSetting('ui.autoOpenCurrentHoverInIntegratedDocs', true)
        const doc = deps.hoverProvider.getLastDoc()
        if (doc?.url && deps.docsBrowserMode() === 'integrated') {
          deps.docsPanelShow(doc.url)
        }
      }
      vscode.window.showInformationMessage(`PyHover Learn Mode ${enabled ? 'enabled' : 'disabled'}.`)
    }),
  )
  ...
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.clearCache', cacheCommands.clearCache),
  )
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.clearStdlibCorpus', cacheCommands.clearStdlibCorpus),
  )
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.clearAllCache', cacheCommands.clearAllCache),
  )
  deps.context.subscriptions.push(
  ...
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.buildPythonCorpus', cacheCommands.buildPythonCorpus),
  )
  deps.context.subscriptions.push(
    vscode.commands.registerCommand('python-hover.openCacheFolder', cacheCommands.openCacheFolder),
  )
}
